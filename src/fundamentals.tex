...
\end{lemma}
\begin{prf}
    Let $S$ be the set of pairs $(v, e)$ where $v \in \v$ and $e \in \e$, which is incident to $v$.
...
\end{proposition}
\begin{prf}
    We will do an induction proof on $\nv$, the \textbf{base case} is the one in which we have a single edge
    \begin{figure}[h]
        \centering
    ...
    Removing vertices with low degree increases the edge density. Let $v \in \v$ with $\deg{v} \leq \ed = \frac{m}{n}$
